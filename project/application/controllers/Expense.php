...
class Expense extends CI_Controller
{
    ...
    public function create_action() 
    {
		...
		'stayinclass_asset_id' => $this->input->post('stayinclass_asset_id',TRUE),
           ...
           $this->Expense_model->update($id,$dataupdate);
              ...
              $this->income_register($data);

           	$this->session->set_flashdata('message', $msg);
            redirect(site_url('expense/success'));
           }
          	 
        }
    }
      public function income_register($data1)
      {       $data = array(
               'employee_id' =>$data1['paying_employee'],
          	   'expense' => $data1['amount_paid'],
	...
	foreach ($this->Expense_model->get_all() as $data) {
            $kolombody = 0;
	    ...
	    xlsWriteNumber($tablebody, $kolombody++, $data->subcategory);
	    xlsWriteLabel($tablebody, $kolombody++, $data->Item_detail);
	    xlsWriteNumber($tablebody, $kolombody++, $data->paying_employee);
	    xlsWriteLabel($tablebody, $kolombody++, $data->amount_paid);
	    ...
	    xlsWriteLabel($tablebody, $kolombody++, $data->room_type);
	    xlsWriteNumber($tablebody, $kolombody++, $data->sic_bill);
	    ...
	    xlsWriteNumber($tablebody, $kolombody++, $data->stayinclass_asset_id);
